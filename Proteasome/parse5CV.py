...
'''
Скрипт делают сводку результатов 5-кратной кросс-валидации для моделей протеасомы.
...
if args.type == "internal":
    ...
    tbl = pd.DataFrame(columns=["model_name", "window", "fold", "descriptor_level", "iap", "twentyCV", "activity", "num_subst"])
    for r in results:
        with open(r, "r", encoding='cp1252') as f:
            split_name = re.split("_", os.path.splitext(os.path.basename(r))[0])
            model_name = split_name[2]
            level = split_name[5]
            fold = split_name[4]
            window = split_name[3]
            lines = f.readlines()
    ...
    for k, folds in models.items():
        comps = re.split("_",k)
        model_name = f"{comps[0]}_{comps[1]}"
        level = int(comps[2])
        union = pd.DataFrame()
        for f in folds:
